...
export async function setMenuButton(webAppUrl) {
...
export async function sendEveningTask(chatId, dayNum, task, miniAppUrl) {
  const eduLine = task.edu_text ? `\n\n💡 <i>Чи знала ти?</i>\n${task.edu_text}` : '';
  await sendMessage(chatId,
    `🌙 <b>День ${dayNum} — завдання на ніч</b>\n\n${task.task_text}${eduLine}`,
...
export async function askSkipReason(chatId, dayNum) {
  await sendMessage(chatId,
    ...
    [
      ...
      [{ text: '📝 Інше', callback_data: `reason_other_${dayNum}` }],
...
export async function sendCheckinSkipComplete(chatId, miniAppUrl) {
  await sendMessage(chatId,
    '💙 Нічого страшного — завтра новий шанс!\n\nПереглянь свою статистику в Mini App:',
    openMiniAppButton('Відкрити прогрес →', miniAppUrl)
  );
}

export async function sendFinalReport(chatId, stats, miniAppUrl) {
  const { doneDays, avgSleep, avgQuality, streak, topReason } = stats;
  const topReasonLine = topReason ? `\nТоп перешкода: ${topReason}` : '';
  await sendMessage(chatId,
    `🎉 Вітаю! Ти пройшла 14-денний челендж!\n\n📊 Твої результати:\n✅ Виконано завдань: ${doneDays} з 14\n😴 Середній сон: ${avgSleep} год\n⭐ Середня якість: ${avgQuality} / 5\n🔥 Найдовша серія: ${streak} днів${topReasonLine}\n\nТи молодець! Продовжуй дотримуватись свого плану сну 💚`,
    openMiniAppButton('Переглянути статистику →', miniAppUrl)
  );
}
